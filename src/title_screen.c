...
#define LEGENDARY_MARKING_PAL_SLOT (BG_PLTT_ID(14) + 1)
#define LEGENDARY_MARKING_YELLOW_B 18
...
{
    if ((frameNum % 2) == 0) // Change color every 4th frame
    {
        u8 coeff = (16 - Cos(frameNum, 16)) / 2;
        u32 b = 31 - (((31 - LEGENDARY_MARKING_YELLOW_B) * coeff) >> 4);

        u16 color = RGB(31, 31, b);
        LoadPalette(&color, LEGENDARY_MARKING_PAL_SLOT, sizeof(color));
    }
}
