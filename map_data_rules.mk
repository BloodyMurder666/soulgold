...
EVENT_SCRIPTS_FILTER := $(TOOLS_DIR)/mapjson/filter_event_scripts.py
FILTERED_EVENT_SCRIPTS := $(DATA_ASM_BUILDDIR)/event_scripts.filtered.s
...
$(FILTERED_EVENT_SCRIPTS): $(DATA_ASM_SUBDIR)/event_scripts.s $(MAPS_DIR)/map_groups.json $(EVENT_SCRIPTS_FILTER)
	python3 $(EVENT_SCRIPTS_FILTER) $< $(MAPS_DIR)/map_groups.json $@

$(DATA_ASM_BUILDDIR)/event_scripts.o: $(FILTERED_EVENT_SCRIPTS)
	$(PREPROC) -s $< charmap.txt | $(CPP) $(CPPFLAGS) $(INCLUDE_SCANINC_ARGS) - | $(PREPROC) -ie $(DATA_ASM_SUBDIR)/event_scripts.s charmap.txt | $(AS) $(ASFLAGS) -o $@
...
$(LAYOUTS_OUTDIR)/layouts.inc $(LAYOUTS_OUTDIR)/layouts_table.inc $(INCLUDECONSTS_OUTDIR)/layouts.h: $(LAYOUTS_DIR)/layouts.json $(MAPS_DIR)/map_groups.json $(MAP_JSONS)
	$(MAPJSON) layouts emerald $< $(MAPS_DIR)/map_groups.json $(LAYOUTS_OUTDIR) $(INCLUDECONSTS_OUTDIR)
