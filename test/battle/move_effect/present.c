...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Present healing through Wonder Guard is still considered to have affected the target (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MIRROR_MOVE) == EFFECT_MIRROR_MOVE);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_SHEDINJA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_WONDER_GUARD); HP(1); MaxHP(100); }
    } WHEN {
        TURN { MOVE(player, MOVE_PRESENT, WITH_RNG(RNG_PRESENT, 254)); }
        TURN { MOVE(opponent, MOVE_MIRROR_MOVE, WITH_RNG(RNG_PRESENT, 1)); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Present healing is blocked by Telepathy on an ally target (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET) { Ability(ABILITY_SHADOW_TAG); Innates(ABILITY_TELEPATHY); HP(50); MaxHP(100); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_PRESENT, target: playerRight, WITH_RNG(RNG_PRESENT, 254)); }
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Present with Parental Bond hits twice when damaging, but only once when healing (Items)")
{
    GIVEN {
        ASSUME(GetSpeciesAbility(SPECIES_KANGASKHAN_MEGA, 0) == ABILITY_PARENTAL_BOND);
